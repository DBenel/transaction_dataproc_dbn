...
def transaction_user_agg(transaction_df):
...
def process_multiple_summary():
    ...
    transaction_master_df = spark.read.format('bigquery') \
    ...
    transaction_user_agg_df = transaction_master_df \
        .transform(lambda df:  transaction_user_agg(df))
    
    transaction_country_agg_df = transaction_master_df \
        .transform(lambda df:  transaction_cp_agg(df, "country_id")) \
        .transform(lambda df:  transaction_country_window(df))
    
    transaction_product_agg_df = transaction_master_df \
        .transform(lambda df:  transaction_cp_agg(df, "product_description"))
    
    transaction_user_agg_df.write \
        .format("bigquery") \
        .option("table", f"{project_id}.{data_set_master}.summary_user") \
        .mode("append") \
        .save()
    
    transaction_country_agg_df.write \
        .format("bigquery") \
        .option("table", f"{project_id}.{data_set_master}.summary_country") \
        .mode("append") \
        .save()
    
    transaction_product_agg_df.write \
        .format("bigquery") \
        .option("table", f"{project_id}.{data_set_master}.summary_product") \
        .mode("append") \
        .save()


if __name__ == "__main__":
    process_multiple_summary()
